...
def loss_batch(model, loss_function, data, labels, freq,opt = None):
    # model_res = model(data)
    # print(model_res.shape)
    # print(labels.shape)
    loss = loss_function(model(data).flatten(), labels)
    loss = 0
    result = model(data).flatten()
    for i in range(len(labels)):
        angle = int(abs(labels[i])//6)
        if angle > 20:
            angle = 21
        loss += loss_function(result[i], labels[i]) * PIC_NUM / freq[angle]
    # print(loss, end =" " )
...
def fit(train_dl, val_dl, model, opt, loss_fun, epochs, freq):
    for epoch in range(epochs):
        ...
        for i,batch in enumerate(train_dl):
            ...
            loss_batch(model, loss_fun, data, labels, freq,opt)
            # print(i, end=" ")
            with torch.no_grad():
                value, num = loss_batch(model, loss_fun, data, labels, freq)
                acc = (num*value + processed * acc)/(num + processed)
                processed += num

        print("Epoch: {}/{} \t Training set accuracy: {:.5f}".format(epoch+1, epochs, acc))

        evaluate(val_dl, model, epoch, epochs, loss_fun, freq)

def evaluate(val_dl, model, epoch, epochs, loss_function, freq):
    ...
    with torch.no_grad():
        ...
        for batch in val_dl:
            ...
            data = data.to(dev)
            labels = labels.to(dev)
            value, num = loss_batch(model, loss_function, data,labels, freq)
            acc = (num*value + processed * acc)/(num + processed)
            processed += num
...
def  main():
    ...
    o1 = model(tst)
    fit(trn_loader, val_loader,model, opt, nn.MSELoss(), args.epochs, freq)
    o2 = model(tst)
    print(o1, o2, lab)
